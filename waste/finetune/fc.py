...
def main(args):
    ...
    class visual_model(nn.Module):
        def __init__(self):
            ...
            self.visual = model.visual.to(dtype=torch.float32)
    ...
    ###断点续连
    # visual_model.load_state_dict(torch.load('/model/zfr888/ljt/LMPT/waste_fc_epoch31/fc_voc-lt_btz_8.pt'))
    '''
    ...
    for epoch in range(args.epochs):
        ...
        with torch.no_grad():
            ...
            if current_mAP > best_mAP:
                ...
                if args.from_scratch is True:
                    torch.save(visual_model.state_dict(), f'/model/zfr888/ljt/LMPT/waste_fc/fc_{args.dataset}_btz_{args.batch_size}_scratch.pt') 
                    print(f'checkpoint saved at /model/zfr888/ljt/LMPT/waste_fc/fc_{args.dataset}_btz_{args.batch_size}_scratch.pt')
                else:
                    torch.save(visual_model.state_dict(), f'/model/zfr888/ljt/LMPT/waste_fc/fc_{args.dataset}_btz_{args.batch_size}.pt') 
                    print(f'checkpoint saved at /model/zfr888/ljt/LMPT/waste_fc/fc_{args.dataset}_btz_{args.batch_size}.pt')
