...
"""
test epoch[15/100] loss:0.072 test mAP:0.8459814859954589
checkpoint saved at /model/zfr888/ljt/LMPT/bert_resnet/bert_resnet50_voc-lt_btz_8.pt
head APs: {'chair': 0.6270168289169035, 'dog': 0.9163584552789273, 'car': 0.9162507097568332,'person': 0.9291485132773708}, 
 middle APs: {'aeroplane': 0.9544374773841039, 'bicycle': 0.8818032109791665, 'bird': 0.9302125816338456, 
 'bottle': 0.608131308264278, 'cat': 0.9326071486106773, 'diningtable': 0.7216846105430333, 'horse': 
 0.9104564195635737, 'motorbike': 0.8574618408390291, 'pottedplant': 
 0.679575012032622, 'sofa': 0.7110488351106905, 'train': 0.9607476132850555, 'tvmonitor': 0.8148604937372217},
  tail APs: {'sheep': 0.8888345880868131, 'cow': 0.8862093717057853, 'boat': 0.9305791085057814, 'bus': 0.8622055923974676}
"""
...
def collate_fn(data):
    ...
    inputs = torch.tensor([item.cpu().detach().numpy() for item in inputs])
...
def main(args):
    ...
    class base_resnet(nn.Module):
        def __init__(self):
            ...
            self.linear1 = nn.Linear(2048,20)
    ...
    '''
    train_dataset =  Voc2007Classification('/data/zfr888/ljt/voc', 'trainval', inp_name='./voc_glove_word2vec.pkl')
    test_dataset = Voc2007Classification('/data/zfr888/ljt/voc', 'test', inp_name='./voc_glove_word2vec.pkl')
    ...
    for epoch in range(args.epochs):
        ...
        with torch.no_grad():
            ...
            if current_mAP > best_mAP:
                ...
                if args.from_scratch is True:
                    torch.save(model.state_dict(), f'/model/zfr888/ljt/LMPT/voc/resnet50_{args.dataset}_btz_{args.batch_size}_scratch.pt') 
                    ...
                    torch.save(model.state_dict(), f'/model/zfr888/ljt/LMPT/voc/resnet50_{args.dataset}_btz_{args.batch_size}.pt') 
