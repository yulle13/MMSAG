...
def ltAnalysis(mAP_cls, dataset):
    ...
    class_freq = np.array([238, 243, 330, 181, 244, 186, 713, 337, 445, 141, 200, 421, 287, 245, 2008, 245, 96, 229, 261, 256])###trainval
    hids = set(np.where(class_freq>=400)[0])
    mids = set(np.where((class_freq<400) * (class_freq >= 200))[0])
    tids = set(np.where(class_freq<200)[0])
