...
####################################
"""
test epoch[14/100] loss:0.246 test mAP:0.40664655986874365
checkpoint saved at /model/zfr888/ljt/LMPT/bert_resnet/weightbert_resnet_mAP:0.40664655986874365_voc-lt_btz_8_scratch.pt
head APs: {'waste_bag': 0.6973739323388919, 'plastic': 0.6455457340976715}, middle APs: {'bottle': 0.528211606750729, 'carton': 0.36703072756435606, 'lile': 0.43832067437960615}, tail APs: {'metal': 0.18510995606483258, 'shoe': 0.18543968857182722, 'galss': 0.20614015918203488}
mAP head: 0.6714598332182817, mAP medium: 0.44452100289823043, mAP tail: 0.19222993460623158
"""
####################################
...
def main(args):
    ...
    class base_resnet(nn.Module):
        ...
        def forward(self, x):
            ...
            x = self.linear(2048,1024)
            x = nn.ReLU()
            x = slef.linear(1024,8)
    ...
    if args.from_scratch is True:
        ...
        optimizer = optim.SGD(model.parameters(), lr=0.1, momentum=0.9, weight_decay=100)
    ...
    for epoch in range(args.epochs):
        ...
        with torch.no_grad():
            ...
            if current_mAP > best_mAP:
                ...
                if args.from_scratch is True:
                    torch.save(model.state_dict(), f'/model/zfr888/ljt/LMPT/bert_resnet/resnet101_{args.dataset}_btz_{args.batch_size}_scratch.pt')
