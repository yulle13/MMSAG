...
from only_resnet import *



####################################
"""""
    test epoch[9/100] loss:0.252 test mAP:0.417502407776006
    checkpoint saved at /model/zfr888/ljt/LMPT/bert_resnet/weightbert_resnet_mAP:0.417502407776006_voc-lt_btz_8_scratch.pt
    head APs: {'waste_bag': 0.6733029622547215, 'plastic': 0.7431749935243175}, middle APs: {'bottle': 0.5501408637921705, 'carton': 0.3540106954943356, 'lile': 0.4758205589947926},         tail APs: {'metal': 0.16137484805372446, 'shoe': 0.18204321825648773, 'galss': 0.2001511218374978}
    mAP head: 0.7082389778895195, mAP medium: 0.45999070609376624, mAP tail: 0.18118972938257003
"""
####################################
...
def main(args):
    ...
    if args.from_scratch is True:
        ...
        optimizer = torch.optim.SGD(model.get_config_optim(0.01, 1.25),
                                lr=0.01,
                                momentum=0.9,
                                weight_decay=1e-4)
    ...
    for epoch in range(args.epochs):
        ...
        with torch.no_grad():
            ...
            if current_mAP > best_mAP:
                ...
                if args.from_scratch is True:
                    torch.save(model.state_dict(), f'/model/zfr888/ljt/LMPT/bert_resnet/resnet50_{args.dataset}_btz_{args.batch_size}_scratch.pt')
